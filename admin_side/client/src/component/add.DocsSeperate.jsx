...
const AddDocsSeperate = () => {
    ...
    useEffect(() => {
        if (selectData) {
            setFormData((prev) => ({
                ...
                swname: selectData?.swname || "",
                swtechnicalEmail: selectData.swtechnicalEmail || "",
                swphone: selectData.swphone || "",
                isMailSent: selectData?.isMailSent || ""
            }));
            setDocs((prev) => ({
                ...prev,
                CustomerDevDocuments: selectData?.CustomerDevDocuments,
                SIEVPLDevDocuments: selectData?.SIEVPLDevDocuments,
                swDevDocumentsforFat: selectData?.swDevDocumentsforFat,
                inspectionDocuments: selectData?.inspectionDocuments,
                dispatchDocuments: selectData?.dispatchDocuments,
                PostCommisionDocuments: selectData?.PostCommisionDocuments,
                SIEVPLDevDocumentsRemarks: selectData?.SIEVPLDevDocumentsRemarks,
                CustomerDevDocumentsRemarks: selectData?.CustomerDevDocumentsRemarks,
                lotval: selectData?.lotval,
            }));
        }
    }, [selectData]);
    ...
    useEffect(() => {
        const getProjectDocsnew = async () => {
            try {
                const val = await fetchProjectsAllnewDocsbyId(id);
                if (val) {
                    setSelectData(val?.docsProject);
                }
            } catch (error) {
...
export default AddDocsSeperate;
