...
export const SelectField = ({ label, value, onChange, options, required = false }) => (
    <div className="flex flex-col space-y-2">
        ...
        <div className="relative">
            ...
            >
                {options.map(option => (
                    <option key={option.value} value={option.value} className="bg-white text-gray-800 py-2">
                        {option.label}
                    </option>
                ))}
            </select>
...
//             errors.push(`${label}: End date cannot be earlier than start date`);
//         }

//         if (completed) {
//             if (!startDate) errors.push(`${label}: Start date is required when completed`);
//             if (!endDate) errors.push(`${label}: End date is required when completed`);
//             if (!daysConsumed || daysConsumed === "0") {
//                 errors.push(`${label}: Days consumed is required when completed`);
//             }
//         }
//     };

//     validateRow(formData.project.rows?.[0], "Project");
//     formData.document.rows.forEach((row, i) => validateRow(row, `Document row ${i + 1}`));
//     formData.screen.rows.forEach((row, i) => validateRow(row, `Screen row ${i + 1}`));
//     formData.logic.rows.forEach((row, i) => validateRow(row, `Logic row ${i + 1}`));
//     formData.testing.rows.forEach((row, i) => validateRow(row, `Testing row ${i + 1}`));
