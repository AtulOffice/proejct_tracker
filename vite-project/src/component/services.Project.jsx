...
import { fetchProjectsSotype } from "../utils/apiCall";
...
const ServicesProj = () => {
  ...
  useEffect(() => {
    ...
    const getProjects = async () => {
      if (debounceSearchTerm && debounceSearchTerm.trim() !== "") {
        try {
          const val = await fetchProjectsSotype({
            page: currentPage,
  ...
  return (
    <div className="max-w-8xl min-h-[140vh] ml-60 px-6 py-12 bg-gradient-to-br from-gray-50 to-white rounded-2xl shadow-sm">
      <h2 className="text-3xl font-bold text-gray-800 my-8 ml-10">SERVICES</h2>
