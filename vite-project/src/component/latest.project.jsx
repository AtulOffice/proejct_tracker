...
import { fetchProjects, fetchProjectslatest } from "../utils/apiCall";
...
const ProjectLatest = () => {
  ...
  useEffect(() => {
    const getProjects = async () => {
      if (debounceSearchTerm && debounceSearchTerm.trim() !== "") {
        try {
          const val = await fetchProjectslatest({
            page: currentPage,
        ...
        const val = await fetchProjects({ page: currentPage, search: "" });
        if (val?.data) {
          setData(val.data);
        }
        if (val?.hashMore !== undefined) {
  ...
  return (
    <div className="max-w-8xl min-h-[140vh] lg:ml-60 px-6 py-12 bg-gradient-to-br from-gray-50 to-white rounded-2xl shadow-sm">
      <h2 className="text-3xl font-bold text-gray-800 my-8 ml-10">LATEST</h2>
      <FilterCompo
